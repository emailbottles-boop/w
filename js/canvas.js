...
function initCanvas() {
    ...
    if (mapCanvas) {
        ...
        mapCanvas.addEventListener('mouseleave', handleMapCanvasMouseUp);
    }
...
function handleCanvasMouseDown(e) {
    ...
    const worldX = (canvasX - canvas.width / 2) / zoom - panX + canvas.width / 2;
    const worldY = (canvasY - canvas.height / 2) / zoom - panY + canvas.height / 2;
    
    if (e.button === 1 || (e.button === 0 && e.ctrlKey) || e.button === 2) {
        // Pan with middle mouse, ctrl+click, or right-click
        e.preventDefault();
        isPanning = true;
        lastPanX = e.clientX;
        lastPanY = e.clientY;
    ...
    if (drawMode !== DRAW_MODES.NONE && isDM) {
        ...
        if (drawMode === DRAW_MODES.PEN) {
            drawOnMap(worldX, worldY, worldX, worldY);
        }
    }
...
function handleCanvasMouseMove(e) {
    ...
    if (drawMode === DRAW_MODES.PEN || drawMode === DRAW_MODES.ERASER) {
        drawOnMap(lastX, lastY, worldX, worldY);
        lastX = worldX;
        lastY = worldY;
    }
...
function importMapToGrid() {
    ...
    // Draw the map canvas content to the grid cell, scaled to fit
    cellCtx.drawImage(mapCanvas, 0, 0, mapCanvas.width, mapCanvas.height, 0, 0, CELL_SIZE, CELL_SIZE);
    ...
    showNotification('Map imported to grid cell [' + (currentCellX + 1) + ', ' + (currentCellY + 1) + ']!', 'success');
}
